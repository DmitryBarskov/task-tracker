...
RSpec.describe 'Edit a task', type: :feature do
  ...
  let(:task) { create(:task, project: project, title: 'A test task!!!!!!!!!', user_id: current_user.id) }
  ...
  context 'when user is not author of the task' do
    let(:project) { create(:project, user: current_user) }
    let(:task) { create(:task, project: project, title: 'A test task!!!!!!!!!', user_id: current_user.id + 1) }
